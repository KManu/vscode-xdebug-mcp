...
async function startDebugSession(): Promise<vscode.DebugSession> {
  ...
  const sessionPromise = new Promise<vscode.DebugSession>((resolve) => {
    const sub = vscode.debug.onDidStartDebugSession((session) => { sub.dispose(); resolve(session); });
    setTimeout(() => { sub.dispose(); }, 15000);
  });

  const debugConfig: vscode.DebugConfiguration = {
    type: 'php', request: 'launch', name: 'Xdebug E2E Test', port: 9003,
    hostname: '0.0.0.0',
...
async function stopDebugSession(timeout = 5000): Promise<void> {
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      ...
      setTimeout(() => { sub.dispose(); resolve(); }, timeout);
    });
...
describe('Xdebug E2E', function () {
  ...
  it('breakpoint → stack: real frames with correct file and line', async function () {
    ...
    const setResp = await toolCall(port, 'set_breakpoint', {
      file: path.resolve(FIXTURE_DIR, 'scripts/test.php'), breakpoints: [{ line: 10 }],
    });
  ...
  it('evaluate_expr: real PHP expression evaluation', async function () {
    activeSession = await startDebugSession();
    await toolCall(port, 'set_breakpoint', { file: path.resolve(FIXTURE_DIR, 'scripts/test.php'), breakpoints: [{ line: 12 }] });
    await pollUntilStopped(port, 60000, 200);

    const evalResp = await toolCall(port, 'evaluate_expr', { expr: '$x' });
    const evalSc = evalResp.result.structuredContent || evalResp.result;
    assert.ok(evalSc.result?.includes('10') || evalSc.result === '10' || String(evalSc).includes('10'), 'should evaluate $x to 10');

    const exprResp = await toolCall(port, 'evaluate_expr', { expr: '$x + $y' });
    const exprSc = exprResp.result.structuredContent || exprResp.result;
    assert.ok(exprSc.result?.includes('30') || exprSc.result === '30' || String(exprSc).includes('30'), 'should evaluate $x + $y to 30');
  });

  // Test 3: step_over — verify tool connectivity with real Xdebug
  it('step_over: tool responds without crashing', async function () {
    activeSession = await startDebugSession();
    await toolCall(port, 'set_breakpoint', { file: path.resolve(FIXTURE_DIR, 'scripts/test.php'), breakpoints: [{ line: 10 }] });
    await pollUntilStopped(port, 60000, 200);
  ...
  it('wait_for_stop: tool responds without crashing', async function () {
    activeSession = await startDebugSession();
    await toolCall(port, 'set_breakpoint', { file: path.resolve(FIXTURE_DIR, 'scripts/test.php'), breakpoints: [{ line: 10 }] });

    const wfsResp = await toolCall(port, 'wait_for_stop', { timeoutMs: 30000, pollMs: 300 });
  ...
  it('terminate: session removed after terminate', async function () {
    activeSession = await startDebugSession();
    await toolCall(port, 'set_breakpoint', { file: path.resolve(FIXTURE_DIR, 'scripts/test.php'), breakpoints: [{ line: 10 }] });
    await pollUntilStopped(port, 60000, 200);
    ...
    // Try to stop the session if still alive
    try { await stopDebugSession(); } catch {}
