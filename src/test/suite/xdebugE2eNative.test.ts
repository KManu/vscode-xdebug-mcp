...
describe("Xdebug E2E Native", function () {
	...
	it("breakpoint → stack: real frames with correct file and line", async () => {
		...
		assert.ok(
			topFrame.source?.path?.includes("test.php"),
			"source should be test.php",
		);
	});
	...
	it("terminate: session removed after terminate", async () => {
		...
		const found = (listSc.sessions || []).some(
			(s: any) => s.id === activeSession!.id,
		);
		assert.strictEqual(
			found,
			false,
			"session should not be in list_sessions after terminate",
		);
	});
});
