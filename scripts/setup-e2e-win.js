...
function findVsCode() {
	...
	try {
		const where = cp
			.execSync("where code", { encoding: "utf8" })
			.trim()
			.split(/\r?\n/);
...
function ensureAdapterInstalled(cliPath) {
	...
	console.log(
		`[setup-e2e-win] installing ${ADAPTER_EXT} into ${path.relative(REPO, EXTENSIONS_DIR)} ...`,
	);
	...
	if (/successfully installed/i.test(out)) {
		...
		console.error(
			`[setup-e2e-win] WARNING: adapter install exit ${r.status}:\n${out.slice(-400)}`,
		);
	}
}
...
function main() {
	...
	if (!vsCodeExe) {
		console.error(
			"[setup-e2e-win] Could not locate VS Code (Code.exe). Set VSCODE_E2E_PATH to its path.",
		);
		process.exit(1);
	}
	...
	const buildDir = fs
		.readdirSync(path.dirname(vsCodeExe))
		.find((d) => /^[0-9a-f]+$/i.test(d));
	const productJson = buildDir
		? path.join(
				path.dirname(vsCodeExe),
				buildDir,
				"resources",
				"app",
				"product.json",
	...
	if (productJson && exists(productJson)) {
		try {
			...
			const ver = p.version;
			const hasMcp = /registerMcpServerDefinition|McpServerDefinition/i.test(
				fs.readFileSync(productJson, "utf8"),
			);
			console.log(
